...
class _VideoDownloadsScreenState extends State<VideoDownloadsScreen> {
  Map<int, List<DownloadedVideo>> downloadedVideoByPara = {};
  bool isLoading = true;
  ...
  Future<void> _loadDownloadedVideos() async {
    ...
    try {
      ...
      if (await sectionDir.exists()) {
          ...
          if (file is File && file.path.endsWith('.mp4')) {
            ...
            if (parts.length >= 3 && parts[2] == 'video') {
              ...
              if (surahIndex != null && ayahIndex != null) {
                ...
                if (!downloadedVideoByPara.containsKey(paraNumber)) {
                  downloadedVideoByPara[paraNumber] = [];
                }
                
                downloadedVideoByPara[paraNumber]!.add(
                  DownloadedVideo(
        ...
        // Sort videos within each para
        downloadedVideoByPara.forEach((para, videos) {
          videos.sort((a, b) {
  ...
  Widget _buildBody(BuildContext context, bool isDark) {
    ...
    if (downloadedVideoByPara.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final videoCount = downloadedVideoByPara[paraNumber]?.length ?? 0;
...
class ParaVideoDetailScreen extends StatelessWidget {
  ...
  Map<int, List<DownloadedVideo>> _groupVideosBySurah() {
    final Map<int, List<DownloadedVideo>> grouped = {};
    
    for (var video in videos) {
      if (!grouped.containsKey(video.surahIndex)) {
        grouped[video.surahIndex] = [];
      }
      grouped[video.surahIndex]!.add(video);
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          final surahIndex = videosBySurah.keys.elementAt(index);
          final surahVideos = videosBySurah[surahIndex]!;
          ...
          return Container(
            ...
            child: ExpansionTile(
              ...
              children: surahVideos.map((video) {
                return ListTile(
                  leading: Container(
                    width: 40,
                    height: 40,
